...
function useFlightResponse(writable, cachePrefix, req, serverComponentManifest, cssFlightData) {
    ...
    if (!entry) {
        ...
        const forwardReader = forwardStream.pipeThrough((0, _nodeWebStreamsHelper).createPrefixStream(cssFlightData)).getReader();
...
function createServerComponentRenderer(ComponentToRender, ComponentMod, { cachePrefix , transformStream , serverComponentManifest , serverContexts  }) {
    ...
    const ServerComponentWrapper = ()=>{
        ...
        const response = useFlightResponse(writable, cachePrefix, reqStream, serverComponentManifest, cssFlightData);
...
function getCssFlightData(ComponentMod, serverComponentManifest) {
    var ref;
    const importedServerCSSFiles = ((ref = ComponentMod.__client__) == null ? void 0 : ref.__next_rsc_css__) || [];
    const cssFiles = importedServerCSSFiles.map((css)=>serverComponentManifest[css].default);
    ...
    const cssFlight = Array.from(cssSet).map((css)=>`CSS:${JSON.stringify({
            chunks: [
                css
            ]
        })}`).join("\n");
...
async function renderToHTML(req, res, pathname, query, renderOpts, isPagesDir) {
    ...
    const createComponentTree = ({ createSegmentPath , tree: [segment, parallelRoutes, { layout , loading , page  }] , parentParams , firstItem  })=>{
        ...
        const parallelRouteComponents = Object.keys(parallelRoutes).reduce((list, currentValue)=>{
            ...
            const childSegmentParam = getDynamicParamFromSegment(parallelRoutes[currentValue][0]);
            const childProp = {
            ...
            list[currentValue] = /*#__PURE__*/ _react.default.createElement(LayoutRouter, {
    ...
    if (isFlight) {
        ...
        const walkTreeWithFlightRouterState = (treeToFilter, parentParams, flightRouterState, parentRendered)=>{
            ...
            if (!parentRendered && renderComponentsOnThisLevel) {
                return [
                    ...
                    /*#__PURE__*/ _react.default.createElement(createComponentTree(// This ensures flightRouterPath is valid and filters down the tree
                    {
            ...
            for (const parallelRouteKey of parallelRoutesKeys){
                ...
                const path = walkTreeWithFlightRouterState(parallelRoute, currentParams, flightRouterState && flightRouterState[1][parallelRouteKey], parentRendered || renderComponentsOnThisLevel);
        ...
        return new _renderResult.default((0, _writerBrowserServer).renderToReadableStream(flightData, serverComponentManifest).pipeThrough((0, _nodeWebStreamsHelper).createPrefixStream(cssFlightData)).pipeThrough((0, _nodeWebStreamsHelper).createBufferedTransformStream()));
    }
    const search = (0, _querystring).stringify(query);
