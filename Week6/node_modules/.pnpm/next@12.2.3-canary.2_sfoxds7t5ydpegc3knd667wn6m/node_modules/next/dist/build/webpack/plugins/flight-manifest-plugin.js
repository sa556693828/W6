    ...
    createAsset(assets, compilation, context) {
        ...
        compilation.chunkGroups.forEach((chunkGroup)=>{
            function recordModule(chunk, id, mod) {
                ...
                if (mod.request && /\.css$/.test(mod.request) && (dev ? mod.loaders.some((item)=>item.loader.includes("next-style-loader/index.js")) : mod.loaders.some((item)=>item.loader.includes("mini-css-extract-plugin/loader.js")))) {
                    if (!manifest[resource]) {
                        if (dev) {
                            const chunkIdNameMapping = (chunk.ids || []).map((chunkId)=>{
                                return chunkId + ":" + (chunk.name || chunk.id) + (dev ? "" : "-" + chunk.hash);
                            });
                            manifest[resource] = {
                                default: {
                                    id,
